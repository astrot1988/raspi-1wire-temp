...
/**
 * Invalid raw data used for testing. The property missing about this data to make it invalid is the
 * fact that it is only one line long.
 */
    ...
    describe('device', function () {
        ...
        describe('_readData', function () {
            it('device does not exist', function () {
                const expectedErrorMessage =
                    '1-Wire compatible file/device does not exist: ' + this.tempDevice.name;

                // Delete the temp file early.
                this.tempDevice.removeCallback();

                const controller = new DS18B20Controller(this.tempDevice.name);
                expect(() => controller._readData()).to.throw(Error, expectedErrorMessage);
            });
        ...
        describe('_parseData', function () {
            ...
            it('can parse data', function () {
                ...
                expect(rawTemperature).to.be.closeTo(VALID_RAW_DATA_TEMP, TEMP_DELTA);
